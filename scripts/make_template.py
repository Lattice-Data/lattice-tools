...
sheet = client.open_by_key('1cHfuuC_wUWai5eTavji3xUWHhCR7GuxHfRX8M-wWYhk')
...
def templatify(schema_name):
	...
	while retry_count < max_retries:
		try:
			...
			all_formats = [('A:A', bold_fmt), ('A1:AZ100', clip_fmt)] + paint_greens + paint_yellows + paint_greys
			format_cell_ranges(tab, all_formats)
			...
			print(f'Successfully created template for {schema_name}')
			return True

		except Exception as e:
			retry_count += 1
			error_msg = str(e)
			print(f'Error processing {schema_name} (attempt {retry_count}): {error_msg}')

			# Check if it's a rate limit error
			if 'quota exceeded' in error_msg.lower() or '429' in error_msg:
				wait_time = base_wait_time * retry_count  # Exponential backoff
				print(f'Rate limit hit. Waiting {wait_time} seconds before retry {retry_count + 1}...')
				time.sleep(wait_time)
			else:
				# For other errors, wait a shorter time
				wait_time = 30 * retry_count
				print(f'Unexpected error. Waiting {wait_time} seconds before retry {retry_count + 1}...')
				time.sleep(wait_time)

			if retry_count >= max_retries:
				print(f'Failed to process {schema_name} after {max_retries} attempts. Aborting.')
				raise Exception(f'Max retries exceeded for {schema_name}')


objtypes = priorityFields.keys()
for i,o in enumerate(objtypes):
	print(f'{o}, {i+1}/{len(objtypes)}')
	templatify(o)
