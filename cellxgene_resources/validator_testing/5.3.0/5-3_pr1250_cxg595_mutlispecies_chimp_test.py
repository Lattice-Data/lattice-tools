...
@pytest.mark.parametrize("test_h5ads", ["valid_chimp.h5ad"])
class TestChimpFixtureObsAndVar:
    @pytest.mark.parametrize(
        "ensembl_prefix_term",
        (
            pytest.param("ZRANB2_ENSPTRG00000000856",id="Cxg unique identifier doesn't start with feature id"),
            pytest.param("NSPTRG00000000856",id="Not a valid prefix for feature id"),
            pytest.param(" ENSPTRG00000000856",id="Not a valid prefix for feature id"),
            pytest.param("ZRANB2",id="Invalid feature id"),
            pytest.param("ENSPTRG00000000856.1",id="Invalid feature id"),
            pytest.param("ENSPTRG00000000856 ",id="Invalid feature id"),
            pytest.param("ENSPTRG-00000000856",id="Invalid feature id"),
            pytest.param("ENSPTRG_00000000856",id="Invalid feature id"),
        )
    )
    def test_feature_ids(self, validator_with_adatas,ensembl_prefix_term):
        ...
        raw_adata.var.replace({'ensembl_id':{'ENSPTRG00000000856':ensembl_prefix_term}},inplace=True)
        raw_adata.var.set_index('ensembl_id',inplace=True)
        validator.adata.raw = raw_adata
        validator.adata.var.reset_index(inplace=True)
        validator.adata.var.replace({'ensembl_id':{'ENSPTRG00000000856':ensembl_prefix_term}},inplace=True)
    ...
    @pytest.mark.parametrize(
        "organism_term",
        (
            pytest.param("NCBITaxon:6239", id="NCBITaxon term for worm not valid with chimp obs"),
            pytest.param("NCBITaxon:7955", id="NCBITaxon term for zebrafish not valid with chimp obs"),
            pytest.param("NCBITaxon:7227", id="NCBITaxon term for fly not valid with chimp obs"),
            pytest.param("NCBITaxon:9606", id="NCBITaxon term for human not valid with chimp obs"),
            pytest.param("NCBITaxon:10090", id="NCBITaxon term for mouse not valid with chimp obs"),
            pytest.param("NCBITaxon:179238", id="NCBITaxon term for mouse descendant not valid with chimp obs"),
        )
    )
...
class TestHumanFixtureChimpTerms:
    @pytest.mark.parametrize(
    ...
    def test_chimp_ncbi_term_in_human_self_reported_ethnicity(self, validator_with_adatas,chimp_term,error_variable):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = chimp_term
        validator.validate_adata()
        assert not validator.is_valid
        assert(
    ...
    def test_chimp_ncbi_term_in_human_dev_stage(self, validator_with_adatas,chimp_term,error_variable):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = chimp_term
        validator.validate_adata()
        assert not validator.is_valid
...
def test_chimp_ncbi_term_in_mouse_dev_stage(validator_with_adatas):
    validator = validator_with_adatas
    validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
    validator.validate_adata()
    assert not validator.is_valid
    assert(
...
class TestWormFixtureChimpTerms:
    def test_chimp_ncbi_term_in_worm_tissue(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_worm_cl(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_worm_dev(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
...
class TestFlyFixtureChimpTerms:
    def test_chimp_ncbi_term_in_fly_tissue(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_fly_cl(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_fly_dev(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
...
class TestZebrafishFixtureChimpTerms:
    def test_chimp_ncbi_term_in_fish_tissue(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_fish_cl(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
    ...
    def test_chimp_ncbi_term_in_fish_dev(self, validator_with_adatas):
        validator = validator_with_adatas
        validator.adata.obs["organism_ontology_term_id"] = "NCBITaxon:9598"
        validator.validate_adata()
        assert not validator.is_valid
