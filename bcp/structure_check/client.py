...
from chebi_lookup.client import BASE, REQUEST_DELAY, get_with_retry, lookup_cas
...
def name_candidates(raw: Any) -> tuple[list[str], list[str]]:
    ...
    pieces = text.split("_")
    if any(piece.strip().casefold() in SALT_TOKENS for piece in pieces):
        return whole, []
...
def cas_structure(cas: Any) -> tuple[str, str]:
    ...
    if not text:
        return "", ""
    properties = lookup_cas(text)
    return (
        properties.get("inchikey") or "",
        properties.get("preferred_name") or "",
    )
