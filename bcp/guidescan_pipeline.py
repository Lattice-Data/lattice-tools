...
class GuidescanPipeline:
    ...
    def validate_and_get_pam_length(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"PAM sequence validated: {pam_sequence} (length: {pam_length})"
            )
    ...
    def validate_inputs(self) -> None:
        ...
        has_real_index = (
            forward_file.exists() and reverse_file.exists() and gs_file.exists()
        )
        ...
        try:
            subprocess.run(["guidescan", "--help"], capture_output=True, check=False)
        except FileNotFoundError:
            raise RuntimeError(
                "guidescan not found in PATH. Please install guidescan2."
            )
    ...
    def run_guidescan(self) -> None:
        ...
        cmd = [
            ...
            "-f",
            str(self.guides_file),
            "-o",
            str(self.all_matches_file),
            "--format",
            "csv",
            "--mode",
            "complete",
        ...
        try:
            ...
            logger.info(
                f"guidescan completed. Results saved to {self.all_matches_file}"
            )
    ...
    def select_best_matches(self) -> None:
        ...
        """
        logger.info(
            "Selecting matches for each guide (retaining all exact matches only)..."
        )
        ...
        try:
            ...
            logger.info(
                f"Processed {guide_count} guides: {exact_match_count} exact matches, {guide_count - exact_match_count} no matches"
            )
    ...
    def format_exact_matches(self) -> pd.DataFrame:
        ...
        try:
                    ...
                    if (
                        ...
                        try:
                            ...
                            if match_distance == 0:
                                ...
                                # Non-exact match
                                results.append(
                                    [id_val, protospacer, pam, "NA", "NA", "NA", "NA"]
                                )
            ...
            logger.info(
                f"Processing complete. Output saved to: {self.final_output_file}"
            )
    ...
    def run(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Summary: {total_guides} guides processed, {exact_matches} exact matches found"
            )
