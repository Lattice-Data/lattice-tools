...
**derived_from:** Each `--raw-subdirs` value resolves to a prefix, whose `raw/` child is listed first; if that yields no CRAMs, the prefix itself is listed. Whichever one matches decides the layout, so no segment name has to be guessed — a group directory holding `raw/{numeric}/` works, a numeric run folder holding `*.cram` directly works, and so does a group directory that itself sits under a top-level `raw/`, or one merely *named* like `ORD01_raw`. Only a prefix whose final segment is exactly `raw` is taken to be at that level already, and it is listed once rather than probed for a `raw/raw/` child. A CRAM counts when it is in a numeric folder one level under the listed prefix, or directly in that prefix when the prefix is itself numeric. Run with `-v` to log which prefix supplied the CRAMs. An `s3://` value must name a directory: a bucket alone is rejected, since searching a whole bucket could match CRAMs from an unrelated delivery. A numeric name such as `426971` is still `…/processed/run_date/` → `…/raw/{subdir}/`. Each deliverable `*.cram` (not `unmatched`) is parsed for `QSR-#`, the well after `QSR-#_` or `QSR-#-` (1–12 + A–H), and whether the name contains `SCALEPLEX`. The well is looked up in rundate `samples.csv` `barcodes`; that sample plus QSR# select `{sample}.QSR-#_anndata.h5ad` or `{sample}.QSR-#-SCALEPLEX.filtered.matrix/matrix.mtx.gz`. The TSV value is `{lab}:{cram_filename}` using `--lab`. A CRAM that does not attach to an output row is printed as a warning. A `--raw-subdirs` entry that matched no CRAMs at all is also printed as a warning, naming every prefix that was tried — the quickest way to spot a typo or a layout the resolved prefix does not describe, since otherwise `derived_from` would just come out empty. `--strict` turns that warning into exit 1.
