...
class TestCSVToBEDDataframe:
    ...
    def test_csv_to_bed_conversion(self, temp_dir):
        ...
        # Check data
        assert (
            bed_df.iloc[0]["chrom"] == "chr1"
        )  # Chromosome normalized with 'chr' prefix
...
class TestAnnotateGuidesBioframe:
    ...
    def test_annotate_guides_statistics(self, temp_dir, guides_file, gtf_file, capsys):
        ...
        assert (
            "Guides without gene overlaps" in captured.out
        )  # Changed from "intergenic regions"
        assert "Total gene overlaps found:" in captured.out
        assert "Output written to:" in captured.out

    def test_multiple_overlaps_creates_multiple_rows(
        self, temp_dir, guides_file, gtf_file
    ):
...
class TestCompatibilityWithCurrent:
    ...
    def test_statistics_format_compatibility(
        self, temp_dir, guides_file, gtf_file, capsys
    ):
