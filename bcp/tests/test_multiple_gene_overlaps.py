...
class TestMultipleGeneOverlaps:
    ...
    def test_minimum_1bp_overlap(self, temp_dir):
        ...
        assert len(overlapping) == 2, (
            f"Expected 2 overlapping guides, got {len(overlapping)}: {overlapping['id'].tolist()}"
        )
        ...
        assert len(non_overlapping) == 2, (
            f"Expected 2 non-overlapping guides, got {len(non_overlapping)}: {non_overlapping['id'].tolist()}"
        )
    ...
    def test_statistics_with_multiple_overlaps(self, temp_dir, capsys):
        ...
        assert (
            "Average overlaps per guide (for guides with overlaps): 1.50"
            in captured.out
        )
