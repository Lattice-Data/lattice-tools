...
guide1,TGCCTCGCGCAGCTCGCGGNGG,3,3000,+,3"""
        ...
        assert result_df.iloc[0]["match_chrm"] == "NA" or pd.isna(
            result_df.iloc[0]["match_chrm"]
        )
        assert result_df.iloc[0]["match_distance"] == "NA" or pd.isna(
            result_df.iloc[0]["match_distance"]
        )
...
guide3,GCCCGCTCCCCGCGATCCCNGG,NA,NA,+,NA,NA,NA,NA,NA"""
        ...
        chromosomes = set(
            str(c).rstrip("0").rstrip(".") for c in guide2_matches["match_chrm"]
        )
        ...
        assert guide3_matches.iloc[0]["match_chrm"] == "NA" or pd.isna(
            guide3_matches.iloc[0]["match_chrm"]
        )
