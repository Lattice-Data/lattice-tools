...
class TestPAMValidation:
    ...
    def test_uniform_pam_ngg(self, temp_dir, dummy_genome_index):
        ...
        assert pipeline.pam_length == 3, (
            f"Expected PAM length 3, got {pipeline.pam_length}"
        )
    ...
    def test_uniform_pam_nngg(self, temp_dir, dummy_genome_index):
        ...
        assert pipeline.pam_length == 4, (
            f"Expected PAM length 4, got {pipeline.pam_length}"
        )
    ...
    def test_uniform_pam_nnngg(self, temp_dir, dummy_genome_index):
        ...
        assert pipeline.pam_length == 5, (
            f"Expected PAM length 5, got {pipeline.pam_length}"
        )
    ...
    def test_pam_length_used_in_sequence_splitting(self, temp_dir, dummy_genome_index):
        ...
        assert len(protospacer) == 19, (
            f"Expected protospacer length 19, got {len(protospacer)}"
        )
