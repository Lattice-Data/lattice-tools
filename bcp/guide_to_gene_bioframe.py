...
def load_genes_gtf(gtf_file):
    ...
    genes_only["gene_id"] = genes_only["attributes"].apply(
        lambda x: extract_attribute(x, "gene_id")
    )
    genes_only["gene_name"] = genes_only["attributes"].apply(
    ...
    for guide_id, original_guide in guide_mapping.items():
        if guide_id not in processed_guide_ids:
            ...
            is_na = (
                chrom == "NA"
                or chrom == "nan"
                or pd.isna(original_guide["chromosome"])
                or start == "NA"
                or start == "nan"
                or pd.isna(original_guide["start"])
                or end == "NA"
                or end == "nan"
                or pd.isna(original_guide["end"])
            )
...
def annotate_guides_bioframe(guide_file, gtf_file, output_file):
    ...
    original_guides_df = pd.read_csv(
        guide_file, dtype=dtype_spec, keep_default_na=False
    )
    ...
    guides_with_genes = (
        len(overlaps_df["name"].unique()) if not overlaps_df.empty else 0
    )
    ...
    print(
        f"  Guides without gene overlaps (intergenic/unmapped/invalid): {guides_without_genes}"
    )
    print(f"  Total gene overlaps found: {total_overlaps}")
    if guides_with_genes > 0:
        print(
            f"  Average overlaps per guide (for guides with overlaps): {total_overlaps / guides_with_genes:.2f}"
        )
