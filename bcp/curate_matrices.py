...
def getArgs():
    ...
    parser.add_argument(
        "--sheet", "-s", help="google sheet id of metadata", required=True
    )
    parser.add_argument(
        "--project",
        "-p",
    ...
    parser.add_argument(
        "--csvofguidescan", "-c", help="Guidescan output CSV", required=True
    )
...
def custom_var_to_obs(adata):
    ...
    for gene_index in adata.var[
        ...
        & (~adata.var["gene_ids"].str.startswith("ENS"))
    ].index:
        gene_id = adata.var.loc[gene_index]["gene_ids"]
        gene_values = adata.X[:, adata.var.index.get_loc(gene_index)]
        adata.obs[gene_id] = (
            gene_values.A.flatten()
            if hasattr(gene_values, "A")
            else gene_values.toarray().flatten().tolist()
        )
...
def gather_crispr(samp):
    ...
    if os.path.isfile(f"temp_cellranger/{samp}_protospacer_calls_per_cell.csv"):
        df = pd.read_csv(
            f"temp_cellranger/{samp}_protospacer_calls_per_cell.csv"
        ).rename(columns={"num_umis": "num_umis_guide_id"})
    else:
        with tarfile.open(
            f"temp_cellranger/{samp}_crispr_analysis.tar.gz", "r:gz"
        ) as tar:
            f = tar.extractfile("protospacer_calls_per_cell.csv")
            df = pd.read_csv(f).rename(columns={"num_umis": "num_umis_guide_id"})
    df["genetic_perturbation_id"] = df["feature_call"].apply(
        lambda x: x.replace("|", " || ")
    )
    df["num_umis_guide_id"] = df["num_umis_guide_id"].apply(
        lambda x: x.replace("|", " || ")
    )
    df = df[["cell_barcode", "genetic_perturbation_id", "num_umis_guide_id"]].set_index(
        "cell_barcode"
    )
...
def gather_metrics(samp, lib_samp):
    ...
    df["Metric"] = df.apply(
        lambda x: f"{x['Metric Name']}, {x['Library Type']}, {x['Category']}", axis=1
    )

    probe_barcodes = " || ".join(
        df[(df["Metric Name"] == "Sample ID") & (df["Metric Value"] == samp)][
            "Group Name"
        ].unique()
    )

    df = df[
        (~df["Grouped By"].isin(["Fastq ID", "Probe barcode ID"]))
...
def add_guide_metadata(adata, sheet, guide_gid, guidescan_output):
    ...
    for row in guide_df.itertuples():
        if row.id not in genetic_perturbations.keys():
            ...
            if not pd.isna([row.start, row.end, row.sense]).all():
                chr_loc = (
                    str(row.chromosome).replace("chr", "")
                    + ":"
                    + str(row.start)
                    + "-"
                    + str(row.end)
    ...
    for row in guide_sheet_df.itertuples():
        genetic_perturbations[row.guide_id]["role"] = (
            "targeting" if row.guide_role == "Targeting a Gene" else "control"
        )
        genetic_perturbations[row.guide_id]["protospacer_adjacent_motif"] = (
            row.guide_PAM
        )
...
def determine_perturbation_strategy(adata):
    ...
    """
    adata.obs["genetic_perturbation_strategy_calculated"] = adata.obs[
        "genetic_perturbation_id"
    ]
    adata.obs["genetic_perturbation_strategy_calculated"] = adata.obs[
        "genetic_perturbation_strategy_calculated"
    ].apply(lambda x: x.split(" || ") if pd.notna(x) else "no perturbations")
    adata.obs["genetic_perturbation_strategy_calculated"] = adata.obs[
        "genetic_perturbation_strategy_calculated"
    ].apply(
        lambda x: (
            [adata.uns["genetic_perturbations"][i]["role"] for i in x]
    ...
    adata.obs.loc[
        adata.obs["genetic_perturbation_strategy_calculated"] == "control",
        "genetic_perturbation_strategy",
    ] = "control"
    adata.obs.loc[
        adata.obs["genetic_perturbation_strategy_calculated"] == "no perturbations",
        "genetic_perturbation_strategy",
    ] = "no perturbations"
...
def map_ontologies(sample_df):
    ...
    col_ont_map = {
        "organism": "NCBITaxon",
        "sex": "PATO",
        "self_reported_ethnicity": {"NCBITaxon:9606": "HANCESTRO", "other": "none"},
        "disease": "MONDO",
        "assay": "EFO",
        "development_stage": {
    ...
    for col in col_ont_map:
        ...
        for label in sample_df[col].unique():
            term_id = None
            if (
                col == "disease" and label == "normal"
            ):  # Normal is not in MONDO ontology
                term_id = "PATO:0000461"
            elif label in [
                "unknown",
                "na",
            ]:  # Unknown and na won't be in ontologies, pass along
                ...
                if col == "tissue":
                    ...
                    if (
                        sample_df.loc[sample_df[col] == label, "tissue_type"].tolist()[
                            0
                        ]
                        != "tissue"
                    ):
                ...
                org_term_id = sample_df.loc[
                    sample_df[col] == label, "organism_ontology_term_id"
                ].tolist()[0]
                if org_term_id in col_ont_map[col]:
                    ...
                    if col_ont_map[col]["other"] == "none":
                        ...
                        term_id = ontology_parser.get_term_id_by_label(
                            label, col_ont_map[col]["other"]
                        )
            else:
            ...
            if term_id is None:
                if org_term_id:
                    if org_term_id in col_ont_map[col]:
                        ont_err_lst.append(
                            f"Error: Matching '{col_ont_map[col][org_term_id]}' term id not found for label '{label}' in column '{col}'"
                        )
                    else:
                        ont_err_lst.append(
                            f"Error: Matching '{col_ont_map[col]['other']}' term id not found for label '{label}' in column '{col}'"
                        )
                else:
                    ont_err_lst.append(
                        f"Error: Matching '{col_ont_map[col]}' term id not found for label '{label}' in column '{col}'"
                    )
                map_dict[label] = label
                continue
            ...
            if c == "donor_living_at_sample_collection":
                for val in sample_df[c].unique():
                    if (
                        val != "na"
                        and sample_df.loc[
                            sample_df[c] == val, "organism_ontology_term_id"
                        ].tolist()[0]
                        != "NCBITaxon:9606"
                    ):
                        print(
                            f"ERROR: donor_living_at_sample_collection for non-human data should be 'na' but '{val}' is present"
                        )
                        sys.exit()
    ...
    sample_df.drop(
        columns=[c for c in sample_df.columns if c.startswith("!")], inplace=True
    )
...
if __name__ == "__main__":
    ...
    response = requests.get(url)
    sample_df = pd.read_csv(BytesIO(response.content), comment="#", dtype=str).dropna(
        axis=1, how="all"
    )
    ...
    for o in [i for i in orders if i.startswith(("AN", "NV"))]:
        r = s3client.list_objects(
            Bucket=args.bucket, Prefix=f"{my_dir}{o}", Delimiter="/"
        )
        libs = [p["Prefix"].replace(f"{my_dir}{o}", "") for p in r["CommonPrefixes"]]
        for lib in libs:
            if lib == args.group + "/":
                r = s3client.list_objects(
                    Bucket=args.bucket,
                    Prefix=f"{my_dir}{o}{lib}processed/cellranger/",
                    Delimiter="/",
                )
                dates = [
                    d["Prefix"].replace(f"{my_dir}{o}{lib}processed/cellranger/", "")
                    for d in r["CommonPrefixes"]
                ]
                for d in dates:
                    r = s3client.list_objects(
                        Bucket=args.bucket,
                        Prefix=f"{my_dir}{o}{lib}processed/cellranger/{d}outs/per_sample_outs/",
                        Delimiter="/",
                    )
                    subs = [
                        s["Prefix"].replace(
                            f"{my_dir}{o}{lib}processed/cellranger/{d}outs/per_sample_outs/",
                            "",
                        )
                        for s in r["CommonPrefixes"]
                    ]
                    for s in subs:
                        samples.append(
                            {
    ...
    for s in samples:
        ...
        adata.obs = adata.obs.merge(sample_df, on="sample_name", how="left").set_index(
            adata.obs.index
        )
        adata.uns["organism_ontology_term_id"] = adata.obs[
        ...
        adata.obs["genetic_perturbation_id"] = adata.obs[
            "genetic_perturbation_id"
        ].astype("category")
        adata.obs["genetic_perturbation_id"] = adata.obs[
            "genetic_perturbation_id"
        ...
        order = order.rstrip("/")
        adata.write(
            filename=f"curated_matrices/{lib}__{samp}__{order}__curated.h5ad",
            compression="gzip",
        )
        for file in [
