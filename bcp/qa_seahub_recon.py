...
from qa_seahub_source import IdentityKey, SourceEntry, TrimmedEntry
...
def _row(category: str, **fields: Any) -> dict[str, Any]:
    row: dict[str, Any] = {
        "category": category,
        "wafer": "",
        "ug": "",
...
def reconcile_trimming(
    ...
    """Compare a vendor delivery against a trimmed upload, well by well."""
    fail_counts = fail_counts or {}
    report = TrimmingReport(
        source_total=len(untrimmed_index), trimmed_total=len(trimmed_index)
    )
