...
The formula layer of the InChI string settles it outright: `C27H29NO11.ClH` against `C27H29NO11` is the same principal component with a counterion attached, and that is a salt difference by definition rather than by inference. The strings arrive in the same payloads as the InChIKeys, so this costs **no extra request**, and it is right about multi-component salts, where PubChem's desalted parent is not.

Where either side has no InChI string — ChEBI records none for class terms and R-group entries — both sides are re-resolved to their **desalted parent** via PubChem and compared again. Agreeing parents demote the row to `salt_differs` / `check`.
...
| `dead_questions` | one of the two questions was asked on ≥5 rows and **compared on none of them** | comparisons |
...
- **`dead_questions` is the one that counts comparisons per *question*.** Point `--chebi-column` at a column of class terms while `--name-column` is healthy: ChEBI holds every record, so nothing is unresolved and nothing is unreachable; the name side makes every row `ok`, so `nothing_compared` is false too. The ID question — the headline column — is answered on no row at all, and without this term the run exits 0. Floored on the rows the question was actually asked on, so two retired ChEBI IDs among 117 rows stay a finding rather than becoming a broken run. Both of its numbers come from the same verdict field, so they cannot drift apart the way an attempted/unresolved ratio drawn from two different sources did.
